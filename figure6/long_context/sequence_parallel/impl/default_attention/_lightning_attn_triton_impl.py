...
def _fwd_kv_parallel(
    ...
    kv_index = block_offset + tl.arange(0, CBLOCK)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, s):
        ...
        grid = (b * h, NUM_FBLOCK, NUM_FBLOCK)
        # 根据上面的kv currrent，算出每个block的累计decay的kv
        _fwd_kv_reduce[grid](
            k,
            v,
            s,
...
def lightning_attention(q, k, v, ed, cp_group):
    d = q.shape[-1]
    e = v.shape[-1]
    # arr = f(d)
    if d >= 128:
    ...
    for i in range(n - 1):
        ...
        k1 = k[..., s:e]  # .contiguous()
        # print(output.shape)
        o = lightning_attention_(q1, k1, v, ed)
        output = output + o
...
from meepo.comm.op import all_to_all
from meepo.parallelism.context_parallel import (
    out_layout_after_comm,
    qkv_layout_after_comm,
    qkv_layout_before_comm,
    repeat_kv,
)
from meepo.utils.numerical import ensure_divisibility


def context_parallel_lightning_attention(
    q, k, v, slope_rate, cp_group, use_fused_kernel: bool  # unused
):
    """
    Context parallel version of lightning attention.
    ...
    Args:
        ...
        use_fused_kernel: unused, just for compatibility with attn_impl interface.
    ...
    if cp_size == 1:
        lightning_attention(q, k, v, slope_rate, cp_group)
    else:
        # ----------------------------------------------------------------------------
        # Context parallel
        # ----------------------------------------------------------------------------
        # layout q, k, v and do all-to-all for k, v
        s, b, head_q, d = q.shape
        assert k.shape == v.shape
        head_kv = k.shape[2]
        ensure_divisibility(head_q, cp_size)
        assert (head_kv % cp_size == 0) or (
            cp_size % head_kv == 0
        ), "The number of head_kv should be divisible by cp_size or vice versa."

        # [s // cp, b, h, d] -> [h, s // cp, b, d]
        # 下列操作仅涉及[T, B, H, K] -> [H, T, B, K] + contiguous
        q = qkv_layout_before_comm(q) 
        k = qkv_layout_before_comm(k)
        v = qkv_layout_before_comm(v)

        if (
            (head_q != head_kv)
            and (cp_size % head_kv == 0)
            and (head_kv != cp_size)
        ):
            # Repeat some heads to use a2a instead of gather+reduce
            repeat_times = cp_size // head_kv
            k, v = repeat_kv(k, v, repeat_times)
        ...
        # [h // cp, s, b, d] -> [s, b, h // cp, d]
        k = qkv_layout_after_comm(k, cp_size)
        v = qkv_layout_after_comm(v, cp_size)
        ...
        q = qkv_layout_after_comm(q, cp_size)
        q, k, v = (rearrange(x, 's b h d -> b h s d') for x in (q, k, v))
        outputs = lightning_attention(q, k, v, slope_rate, cp_group)
        outputs = rearrange(outputs, 'b h s d -> s b (h d)')
        outputs = all_to_all(
            outputs,
            cp_group,
            output_split_sizes=None,
        ...
        output_tensor = out_layout_after_comm(outputs, cp_size)
        return output_tensor


# lightning_attention = _attention.apply
