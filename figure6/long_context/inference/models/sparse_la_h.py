...
"""Inference-only SparseLA-H model."""
import copy
import math
import re
...
from vllm.model_executor.layers.sparse_la import (
    sparse_linear_attention, linear_decode_forward_triton)    
...
from .sparse_la_h_cache import SparseLAHCacheManager, SparseLAHCacheParams
...
class SparseLAHRMSNormTP(CustomOp):
    name = "SparseLAHRMSNormTP"
...
class SparseLAHRotaryEmbedding(CustomOp):
    name = "SparseLAHRotaryEmbedding"
...
class SparseLAHMoE(nn.Module):
    ...
    def __init__(
        ...
        self.gate.weight.weight_loader = SparseLAHMoE.gate_weight_loader
...
class SparseLAHLinearKernel:
    ...
    def jit_linear_forward_prefix(q: torch.Tensor,
        ...
        output, kv_history = sparse_linear_attention(q,
                                                 k,
                                                 v,
                                                 slope_rate,
                                                 is_sparse, 
        ...
        kv_caches.copy_(kv_history.reshape(h, d, e)) #reshap h d e
        assert output.shape[0] == 1, "batch size must be 1"
        return rearrange(output.squeeze(0), "h n d -> n (h d)")


class SparseLAHLinearAttention(nn.Module):
    ...
    def __init__(
        ...
        self.norm = SparseLAHRMSNormTP(
            self.hidden_inner_size,
            eps=1e-5,
        )

        slope_rate = SparseLAHLinearAttention._build_slope_tensor(
            self.num_heads)
    ...
    def _prefill_and_mix_infer(self, q, k, v, kv_cache, state_indices_tensor,
        ...
        for _prefill_idx in range(getattr(attn_metadata, "num_prefills", 0)):
            ...
            out_slice = SparseLAHLinearKernel.jit_linear_forward_prefix(
                qs,
                ks,
                vs,
                slice_layer_cache,#kv_cache
                self.tp_slope,#rate
                self.is_sparse,
                self.sparse_rate,
                self.BLOCK,
    ...
    def _decode_infer(self, q, k, v, kv_cache, state_indices_tensor,
        ...
        hidden = linear_decode_forward_triton(q, k, v, kv_cache, self.tp_slope,
                                              slot_id,self.is_sparse,self.sparse_rate, 32)
        return hidden

    def forward(self, hidden_states: torch.Tensor, positions: torch.Tensor,
                kv_caches: SparseLAHCacheParams, **kwargs) -> torch.Tensor:
        ...
        kv_cache = kv_caches.sparse_la_h_cache
...
class SparseLAHAttention(nn.Module):
...
class SparseLAHDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if config.attention_type == 0:
            ...
            self.self_attn = SparseLAHLinearAttention(
                hidden_size=self.hidden_size,
                ...
                quant_config=quant_config,
                is_sparse = config.is_sparse if hasattr(config, "is_sparse") else True,
                sparse_rate = config.sparse_rate if hasattr(config, "sparse_rate") else 0.5,
                layer_idx=self._ilayer,
                linear_layer_idx=linear_layer_id,
                prefix=prefix)
        elif config.attention_type == 1:
            self.self_attn = SparseLAHAttention(
                hidden_size=self.hidden_size,
                num_heads=config.num_attention_heads,
                head_dim=head_dim,
        ...
        if expert_num == 1:
            ...
            self.block_sparse_moe = SparseLAHMoE(
...
class SparseLAHModel(nn.Module):
    ...
    def __init__(
        ...
        def layer_fn(prefix):
            ...
            return SparseLAHDecoderLayer(layer_config,
                                             **decoder_kwargs,
                                             prefix=prefix)
        ...
        self.sparse_la_h_cache = SparseLAHCacheManager(
            dtype=self._dtype,
        ...
        if hasattr(config, "max_model_len") and isinstance(
            ...
            max_position_embeddings = min(config.max_position_embeddings,
                                          config.max_model_len)
        self.rotary_emb = SparseLAHRotaryEmbedding(
            head_dim,
            rotary_dim=config.rotary_dim
            if hasattr(config, "rotary_dim") else head_dim,
            max_position=max_position_embeddings,
    ...
    def _clear_prefill_cache(self, attn_metadata,
                             sparse_la_h_cache_tensors: torch.Tensor, **kwargs):
        seq_to_slot_maps = {}
        seq_id_map = sum(list(kwargs["request_ids_to_seq_ids"].values()), [])
        for _, seq_to_slot_map in (
                self.sparse_la_h_cache.cache_indices_mapping.items()):
        ...
        if slots_to_clear:
            slots_tensor = torch.tensor(slots_to_clear,
                                        device=sparse_la_h_cache_tensors.device,
                                        dtype=torch.long)
            sparse_la_h_cache_tensors[:, slots_tensor, ...] = 0
    ...
    def forward(self,
        ...
        (
            sparse_la_h_cache_tensors,
            state_indices_tensor,
        ) = self.sparse_la_h_cache.current_run_tensors(**kwargs)
        if getattr(attn_metadata, "num_prefills", 0) > 0:
            self._clear_prefill_cache(attn_metadata, sparse_la_h_cache_tensors,
                                      **kwargs)

        sparse_la_h_cache_params = SparseLAHCacheParams(
            sparse_la_h_cache_tensors,
            state_indices_tensor,
        )
        if get_pp_group().is_first_rank:
        ...
        kv_cache_index = 0
        sparse_la_h_cache_index = 0
        attn_metadata.rotary_emb = self.rotary_emb
        for i in range(self.start_layer, self.end_layer):
            layer = self.layers[i]
            _caches = None
            if isinstance(layer.self_attn, SparseLAHAttention):
                #_caches = kv_caches[kv_cache_index]
                kv_cache_index += 1
            if isinstance(layer.self_attn, SparseLAHLinearAttention):
                current_state_layer = sparse_la_h_cache_index
                _caches = sparse_la_h_cache_params.at_layer_idx(
                    current_state_layer)
                sparse_la_h_cache_index += 1
...
class SparseLAHForCausalLM(nn.Module, HasInnerState, IsHybrid,
                               SupportsV0Only):
    ...
    def copy_inputs_before_cuda_graphs(self, input_buffers, **kwargs):
        return self.model.sparse_la_h_cache.copy_inputs_before_cuda_graphs(
            input_buffers, **kwargs)

    def get_seqlen_agnostic_capture_inputs(self, batch_size: int):
        return self.model.sparse_la_h_cache.get_seqlen_agnostic_capture_inputs(
            batch_size)
    ...
    def load_weights(self, weights: Iterable[Tuple[str,
        ...
        def load_linear_attn_weight(name: str, loaded_weight: torch.Tensor,
            ...
            weight_loader = getattr(
                param, "weight_loader",
                SparseLAHLinearAttention.weight_direct_load)
            weight_loader = weight_loader_with_alias(name)(weight_loader)
            weight_loader(param, loaded_weight)
            return
        ...
        for name, loaded_weight in weights:
            ...
            if weight_at_layer is not None and weight_at_layer >= len(
