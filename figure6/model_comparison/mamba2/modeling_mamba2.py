...
class Mamba2Mixer(nn.Module):
                ...
                if causal_conv1d_fn is None or self.activation not in ["silu", "swish"]:
                    hidden_states_B_C = self.act(
                        self.conv1d(hidden_states_B_C.transpose(1, 2)).transpose(
                            1, 2)[:, :seq_len])  # (B, L, self.d_inner + 2 * ngroups * d_state)
                else:
    ...
    def forward(
        ...
        if is_fast_path_available and "cuda" in self.in_proj.weight.device.type:
            return self.cuda_kernels_forward(hidden_states.to(self.in_proj.weight.dtype), cache_params, cache_position,
                                             attention_mask)
        dtype = hidden_states.dtype
        if attention_mask is not None and attention_mask.shape[1] > 1 and attention_mask.shape[0] > 1:
            # tune out hidden states for pad tokens, see https://github.com/state-spaces/mamba/issues/66
...
class Mamba2Block(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon, dtype=torch.float32)
        ...
        if isinstance(module, Mamba2Mixer):
            ...
            dt = torch.exp(
                ...
                * (math.log(self.config.time_step_max) - math.log(self.config.time_step_min))
                + math.log(self.config.time_step_min)).clamp(min=self.config.time_step_floor)
...
class Mamba2Model(Mamba2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.norm_f = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_epsilon, dtype=torch.float32)
        ...
        **kwargs,
        ...
        if use_cache:
            if cache_params is None:
                cache_params = Mamba2Cache(self.config,
        ...
        **kwargs,
        ...
        if use_cache:
            ...
            if cache_position is None:
                raise ValueError("`cache_position` should not be None as it should have been initialized in "
            ...
            if cache_position[0] > 0:
                ...
                # Extend the mask to match the convolution-state history.
                extended_mask = torch.ones(attention_mask.size(0),
        ...
        if attention_mask.shape[1] < past_len:
            ...
            # Extend manually because position IDs are unavailable in this branch.
            extended_mask = torch.ones(attention_mask.size(0),
                                       past_len - attention_mask.shape[1],
                                       device=attention_mask.device)
            attention_mask = torch.cat([attention_mask, extended_mask], dim=1)
