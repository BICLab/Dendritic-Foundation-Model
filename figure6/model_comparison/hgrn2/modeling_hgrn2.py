...
class Hgru2(nn.Module):
    ...
    def forward(self, x, lower_bound=0, state=None):
        ## x: n b d
        n, b, d = x.shape
        ...
        if self.training:
            V, Q, G_K, K = map(
                lambda x: rearrange(self.pad(x), "(n c) b h d -> b h n c d", c=self.chunk_size).contiguous(),
                [V, Q, log_lambda_, K],
            )
            G_V = None
            ...
            V, Q, G_K, K = map(
                lambda x: rearrange(x, "n b h d -> b h n d").to(torch.float32).contiguous(),
                [V, Q, log_lambda_, K],
            )
            # o = fused_recurrent_gla(Q, K, V, G_K)
            o, state = fused_recurrent_gla(Q, K, V, G_K, scale=1, initial_state=state, output_final_state=True)
...
class Hgrn2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        residual = x
        x, state = self.token_mixer(self.token_norm(x), lower_bound, state)
        x = x + residual
...
@add_start_docstrings(HGRN2_START_DOCSTRING, )
...
class Hgrn2Model(Hgrn2PreTrainedModel):
    ...
    def __init__(self, config: Hgrn2Config):
        ...
        self.layers = nn.ModuleList([Hgrn2DecoderLayer(config) for i in range(config.decoder_layers)])
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer(hidden_states, padding_mask, lower_bound, past_key_value)
        ...
        return Hgrn2ModelOutputWithPast(last_hidden_state=hidden_states, past_key_values=cache_values)
