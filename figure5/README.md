...
- **Neuromorphic speech:** convert each one-second spike stream to 250 frames at `dt = 4 ms`, producing `250 × 700` inputs. Train approximately 0.3M-parameter models for 200 epochs with AdamW, cosine decay, weight decay `0.1`, and an initial learning rate selected from `1e-2` or `5e-3`. Results use five runs.
...
Each task has independent dependencies and data-layout assumptions. Follow the task README rather than constructing one shared environment:
