...
def krylov(L, A, b, c=None, return_power=False):
    ...
    """
    # NOTE: There is an edge case if L=1 where output doesn't get broadcasted, which might be an issue if caller is expecting broadcasting semantics... can deal with it if it arises
...
def krylov_toeplitz_(L, A, b, c=None):
    ...
    NOTE: currently not faster than original version, not sure why
    """
    N = A.shape[-1]
