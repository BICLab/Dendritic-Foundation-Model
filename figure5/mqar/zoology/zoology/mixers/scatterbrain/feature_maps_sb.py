...
from zoology.mixers.scatterbrain.projection_utils import gaussian_orthogonal_random_matrix
...
class SBPerformerFeatures(FeatureMap):
    ...
    def __init__(self, query_dims, n_features=None, ortho_scaling=0, softmax_temp=None,
        ...
        # NOTE: we're not using @orthogonal atm
        self.orthogonal = orthogonal
        self.softmax_temp = 1 / math.sqrt(query_dims) if softmax_temp is None else softmax_temp
        self.cosh = cosh
        # self.redraw = redraw
        # NOTE: not redrawing atm, so I'm setting it to an irrational number
        self.redraw = math.pi
