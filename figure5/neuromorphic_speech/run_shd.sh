...
: "${DATA_PATH:?Set DATA_PATH to the directory containing generated SHD arrays.}"
: "${CUDA_VISIBLE_DEVICES:?Set CUDA_VISIBLE_DEVICES to the GPU(s) to use.}"

OUTPUT_DIR="${OUTPUT_DIR:-shd_0.3m}"
...
for seed in $SEEDS; do
  command=(python shd_train.py --data-path "$DATA_PATH" --block "$BLOCK" --seed "$seed")
