# coding=utf-8
"""PyTorch SparseLA model with a legacy module path."""
...
class SparseLAM(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, lamda_ratio=1, mode='fused_chunk', layer_idx=None):
        super().__init__()
        self.mode = mode
        ...
        self.q_proj = nn.Linear(self.embed_dim, self.q_dim, bias=False)
        self.gk_proj =  nn.Linear(self.embed_dim, self.q_dim, bias=False)

        self.v_proj = nn.Linear(self.embed_dim, self.v_dim, bias=False)
        self.g_proj = nn.Linear(self.embed_dim, self.v_dim, bias=True)
        self.out_proj = nn.Linear(self.v_dim, self.embed_dim, bias=False)
...
# Historical class names retained for downstream imports.
spla1d = SparseLAV
spla2d = SparseLAM
...
class SparseLAForCausalLM(SparseLAPreTrainedModel):
    ...
    def forward(
        ...
        >>> from sparsela_hf.modeling_sparsela import SparseLAForCausalLM
