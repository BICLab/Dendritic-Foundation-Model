...
class MS_ConvBlock(nn.Module):
    def __init__(
        ...
        # self.conv_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend="cupy")
        # self.Conv = nn.Conv2d(dim,dim,kernel_size=3,padding=1,stride=1,bias=False)
    ...
    def forward(self, x):
        ...
        # x = self.Conv(self.conv_lif(x).flatten(0,1)).reshape(T,B,C,H,W) + x
        x = self.Conv(x) + x
        x_feat = x
...
class MS_ConvBlock_ablation(nn.Module):
    def __init__(
        self,
        dim,
        mlp_ratio=4.0,
        ...
        self.noSepConv = noSepConv

        if self.noSepConv == False:
            self.Conv = SepConv(dim=dim)
...
class MS_Attention_RepConv_qkv_id(nn.Module):
    def __init__(
        ...
        self.attn_lif = MultiStepLIFNode(
            tau=2.0, v_threshold=4.0, detach_reset=True, backend="cupy"
        )
...
class SDSA_2(nn.Module):
    def __init__(
        ...
        self.qk_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend="cupy")
    ...
    def forward(self, x, hook=None):
        ...
        qk = self.qk_conv(x.flatten(0, 1)).reshape(T,B,C,H,W)
...
class SDSA_4(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.num_heads = num_heads
        self.register_parameter("scale", nn.Parameter(torch.tensor([0.125])))
...
class MS_Attention_RepConv_qkv_id_Ablation(nn.Module):
    def __init__(
        self,
        dim,
        num_heads=8,
        ...
        if qkv_in_linear == False:
            ...
            self.q_conv = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1, bias=False), nn.BatchNorm2d(dim))

            self.k_conv = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1, bias=False), nn.BatchNorm2d(dim))

            self.v_conv = nn.Sequential(nn.Conv2d(dim, dim, kernel_size=1, bias=False), nn.BatchNorm2d(dim))
        ...
        if qkv_out_linear == False:
            self.proj_conv = nn.Sequential(
                RepConv(dim, dim, bias=False), nn.BatchNorm2d(dim)
            )
        else:
            self.proj_conv = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=1, bias=False), nn.BatchNorm2d(dim)
        )
...
class MS_Block(nn.Module):
    def __init__(
        ...
        self.attn = MS_Attention_RepConv_qkv_id_Ablation(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
            qk_scale=qk_scale,
...
class MS_Block_SDSA_Ablation(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
...
class MS_Block_Ablation(nn.Module):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        self.attn = MS_Attention_RepConv_qkv_id_Ablation(
            ...
            proj_drop=drop,
            sr_ratio=sr_ratio,
            qkv_in_linear=qkv_in_linear,
            qkv_out_linear=qkv_out_linear
        )
...
class Spiking_vit_MetaFormer_Ablation(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
        ...
        self.ConvBlock1_1 = nn.ModuleList(
            [MS_ConvBlock_ablation(dim=embed_dim[0] // 2, mlp_ratio=mlp_ratios, noSepConv=noSepConv, Conv1x1=Conv1x1)]
        )
        ...
        self.ConvBlock1_2 = nn.ModuleList(
            [MS_ConvBlock_ablation(dim=embed_dim[0], mlp_ratio=mlp_ratios, noSepConv=noSepConv, Conv1x1=Conv1x1)]
        )
        ...
        self.ConvBlock2_1 = nn.ModuleList(
            [MS_ConvBlock_ablation(dim=embed_dim[1], mlp_ratio=mlp_ratios, noSepConv=noSepConv, Conv1x1=Conv1x1)]
        )
...
class Spiking_vit_MetaFormer_SDSA_ablation(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
class Spiking_vit_MetaFormer_ViT(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
class Spiking_vit_MetaFormer_CNN(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
        ...
        self.ConvBlock1_1 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[0] // 2, mlp_ratio=mlp_ratios[0])]
        )
        ...
        self.ConvBlock1_2 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[0], mlp_ratio=mlp_ratios[0])]
        )
        ...
        self.ConvBlock2_1 = nn.ModuleList(
            [MS_ConvBlock(dim=embed_dim[1], mlp_ratio=mlp_ratios[0])]
        )
        ...
        self.block3 = nn.ModuleList(
            [
                MS_ConvBlock(dim=embed_dim[2], mlp_ratio=mlp_ratios[1])
                for j in range(6)
            ]
        )
        ...
        self.block4 = nn.ModuleList(
            [
                MS_ConvBlock(dim=embed_dim[3], mlp_ratio=mlp_ratios[1])
                for j in range(2)
            ]
        )
...
class Spiking_vit_MetaFormer_Remove_Pryamid(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
class Spiking_vit_MetaFormer_stage1_1_pooling(nn.Module):
    def __init__(
        self,
        img_size_h=128,
        img_size_w=128,
...
def spikformer_8_19M_CAFormer(**kwargs):
    model = Spiking_vit_MetaFormer(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[72, 144, 288, 408],
        num_heads=8,
        mlp_ratios=4,
        in_channels=3,
        num_classes=1000,
...
def spikformer_8_15M_CAFormer_qkv_out_linear(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[64, 128, 256, 360],
...
def spikformer_8_384_CAFormer_baseline(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_noSepConv(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_Conv1x1(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_qkv_in_linear(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_qkv_out_linear(**kwargs):
    model = Spiking_vit_MetaFormer_Ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_CNN(**kwargs):
    model = Spiking_vit_MetaFormer_CNN(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[72, 144, 288, 360],
        # embed_dim=[96, 192, 384, 480],
        num_heads=8,
        mlp_ratios=[4,2],
        in_channels=3,
...
def spikformer_8_384_CAFormer_ViT(**kwargs):
    model = Spiking_vit_MetaFormer_ViT(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[96, 192, 384, 480],
...
def spikformer_8_384_CAFormer_SDSA_1(**kwargs):
    model = Spiking_vit_MetaFormer_SDSA_ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[96, 192, 384, 480],
...
def spikformer_8_384_CAFormer_SDSA_2(**kwargs):
    model = Spiking_vit_MetaFormer_SDSA_ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_SDSA_4(**kwargs):
    model = Spiking_vit_MetaFormer_SDSA_ablation(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
...
def spikformer_8_384_CAFormer_Remove_Pryamid(**kwargs):
    model = Spiking_vit_MetaFormer_Remove_Pryamid(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[96, 192, 384, 384],
        num_heads=8,
        mlp_ratios=4,
        in_channels=3,
        num_classes=1000,
...
def spikformer_8_384_CAFormer_stage1_1_pooling(**kwargs):
    model = Spiking_vit_MetaFormer_stage1_1_pooling(
        img_size_h=224,
        img_size_w=224,
        patch_size=16,
        embed_dim=[96, 192, 384, 480],
...
if __name__ == "__main__":
    model = spikformer_8_10M_CAFormer()
    print(model)
